export const makeStepperStyles = ({ primitives, tokens }) => ({
    MuiStep: {
        styleOverrides: {
            root: {
            ...
            vertical: {
                '& [class*="MuiStepContent-root"]': {
                    marginLeft: "15.5px",
                    borderLeft: `1.5px solid ${tokens.colors["detail-1-transparent"]}`,
    ...
    MuiStepButton: {
        styleOverrides: {
            root: Object.assign(Object.assign({}, tokens.typography["body-copy-medium"]), { "&:hover": {
                    '& [class*="MuiStepLabel-iconContainer"]': {
                        outline: `${primitives.borderWidths.medium} solid ${primitives.colors.tint["iris-400"]}`
                    },
                    '& [class*="Mui-error"]': {
                        '& [class*="MuiStepLabel-iconContainer"]': {
                            outline: `${primitives.borderWidths.medium} solid ${primitives.colors.tint["clay-300"]}`
                        }
                    }
                }, "&:active": {
                    '& [class*="MuiStepLabel-iconContainer"]': {
                        outline: `${primitives.borderWidths.large} solid ${primitives.colors.accent.plant}`
                    },
                    '& [class*="Mui-error"]': {
                        '& [class*="MuiStepLabel-iconContainer"]': {
                            outline: `${primitives.borderWidths.large} solid ${primitives.colors.tint["clay-300"]}`
                        }
                    }
                }, "&:focus-visible": {
                    '& [class*="MuiStepLabel-iconContainer"]': {
    ...
    MuiStepConnector: {
        styleOverrides: {
            ...
            lineHorizontal: {
                borderTop: `1.5px solid ${tokens.colors["detail-1-transparent"]}`
            },
            lineVertical: {
                borderLeft: `1.5px solid ${tokens.colors["detail-1-transparent"]}`
            }
    ...
    MuiStepLabel: {
        styleOverrides: {
            ...
            iconContainer: {
                ...
                borderRadius: "50%",
                boxShadow: `${tokens.colors["ink-on-background"]} 0 0 0 1.5px inset`,
                paddingRight: primitives.spacings["0"],
                marginRight: primitives.spacings["2"],
                '&[class*="Mui-active"]': {
                    boxShadow: `${tokens.colors["ink-on-background"]} 0 0 0 2px inset`,
                    '& [class*="MuiSvgIcon"]': {
                        "& text": {
                ...
                '&[class*="Mui-error"][class*="Mui-active"]': {
                    boxShadow: `${tokens.colors["ink-error"]} 0 0 0 2px inset`,
                    backgroundColor: primitives.colors.detail.transparent,
                    '& [class*="MuiSvgIcon"]': {
                ...
                '&[class*="Mui-disabled"]': {
                    boxShadow: `${tokens.colors["detail-1-transparent"]} 0 0 0 1.5px inset`,
