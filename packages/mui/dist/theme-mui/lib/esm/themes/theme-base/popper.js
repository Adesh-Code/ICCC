export const makePopperStyles = ({ primitives, tokens }) => ({
    MuiPopper: {
        styleOverrides: {
            root: {
                "&.DhigFlyout": {
                ...
                '&[class*="MuiAutocomplete-popper"]': {
                    '& [class*="MuiPaper-root"]': Object.assign(Object.assign({}, tokens.typography["body-copy-medium"]), { color: tokens.colors["ink-on-background"], boxShadow: tokens.effects["elevation-0-dropdown"], borderRadius: `${primitives.borderRadii.none} ${primitives.borderRadii.none} ${primitives.borderRadii.medium} ${primitives.borderRadii.medium}`, margin: `${primitives.spacings["1"]} ${primitives.spacings["0"]}`, backgroundColor: tokens.colors["fill-elevation-0"], '& [class*="MuiAutocomplete-noOptions"]': {
                            color: tokens.colors["ink-on-background"]
                        }, '& [class*="MuiAutocomplete-listbox"]': {
                            '& [class*="MuiAutocomplete-option"]': Object.assign(Object.assign({ cursor: "pointer", display: "flex", boxSizing: "border-box", alignItems: "center", padding: `10px ${primitives.spacings["3"]}`, justifyContent: "flex-start" }, tokens.transitions["state-blur"]), { '&[aria-selected="true"]': {
                                    backgroundColor: tokens.colors["activated-fill-subtle"]
                                }, '&[aria-disabled="true"]': {
                                    opacity: "0.3",
                                    pointerEvents: "none"
                                }, "&.Mui-focused:hover": Object.assign({ backgroundColor: tokens.colors["hover-fill-subtle"] }, tokens.transitions["state-activate"]), "&.Mui-focused:active, &:active": Object.assign({ backgroundColor: tokens.colors["pressed-fill-subtle"] }, tokens.transitions["state-activate"]), "&.Mui-focused.Mui-focusVisible": Object.assign({ boxShadow: `inset ${tokens.effects["focus-ring-thin"]}` }, tokens.transitions["state-activate"]), '& [class*="DhigCombobox--checkedIcon"]': {
                                    position: "absolute",
                                    right: primitives.spacings["3"]
                        ...
                        }, '& [class*="DhigCombobox--addendum"]': Object.assign(Object.assign({ color: tokens.colors["detail-2-transparent"] }, tokens.typography["body-copy-small"]), { padding: primitives.spacings["3"] }) })
                }
            }
        }
    }
